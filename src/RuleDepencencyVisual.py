...
class Atom:
    ...
    def parse(self):
        ...
        self.m_rel.m_data = self.m_dom.getElementsByTagName('Rel')[0].toxml()
...
class Rule:
    ...
    def parse(self):
        ...
        self.m_dom = parseString(self.m_data)
        head = self.m_dom.getElementsByTagName('head')[0].toxml()
        self.m_head = Head(head);
        self.m_head.setRuleId(self.m_id)
        self.m_head.parse()
        body = self.m_dom.getElementsByTagName('body')[0].toxml()
...
class Rule_Library:
        ...
        for ath in self.m_headAtoms:
            for atb in self.m_bodyAtoms:
                if(ath.isSame(atb) == True and ath.ruleId() != atb.ruleId()):
                    ...
                    node_a = pydot.Node("%d" % ath.ruleId(), style="filled", fillcolor="red")
                    node_b = pydot.Node("%d" % atb.ruleId(), style="filled", fillcolor="red")
    ...
    def parse(self):
        self.m_dom = parseString(self.m_data)
        rules = self.m_dom.getElementsByTagName('Rule')
        id = 0;
        for r in rules:
            rule = Rule(r.toxml())
            rule.setId(id)
...
class Parser:
    ...
    def parse(self):
        ...
        self.m_dom = parseString(self.m_data)
        self.m_library = Rule_Library(self.m_dom.getElementsByTagName('Library')[0].toxml())
